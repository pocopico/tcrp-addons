...
function build_config() {
	...
	platforms $model
	if [ $revision -le 60000 ]; then
		cat <<EOF
 "${model}_${revision}":  "https://raw.githubusercontent.com/pocopico/rp-ext/master/redpilldev/releases/${platform}_42218.json",
EOF
	fi
	if [ $revision -ge 60001 ]; then
		cat <<EOF
 "${model}_${revision}":  "https://raw.githubusercontent.com/pocopico/rp-ext/master/redpilldev/releases/${platform}_64216.json",
}
EOF
	fi
}
